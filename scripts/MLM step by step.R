...
setwd("/Users/admin/Desktop/WORK/s-wave-intensity-estimation-nz/data")
eqdata <- read.csv("eqdata_vel_new.csv", check.names = FALSE)
head(eqdata)
str(eqdata)

eqdata$earthquake_id <- factor(eqdata$earthquake_id)
eqdata$trace_name <- factor(eqdata$trace_name)
eqdata$station_name <- factor(eqdata$station_name)
eqdata$year <- factor(eqdata$year)
...
# Null model
model_null <- lmer(log_PGV ~ 1 + (1 | SiteClass) + (1 | station_name) + (1 | earthquake_id) + (1 | year), data = train_data, REML = FALSE, control = control)
summary(model_null)
anova(model_null)
...
var_between_station <- variance_components$vcov[variance_components$grp == "station_name"]
var_between_site_class <- variance_components$vcov[variance_components$grp == "SiteClass"]
var_between_earthquake_id <- variance_components$vcov[variance_components$grp == "earthquake_id"]
var_between_year <- variance_components$vcov[variance_components$grp == "year"]
...
ICC_year <- var_between_year / (var_between_year + var_within)
...
print(paste("ICC for year index:", ICC_year))

# Calculate the average cluster size for each grouping variable
average_cluster_size_station <- mean(tapply(eqdata$log_PGV, eqdata$station_name, length))
average_cluster_size_site_class <- mean(tapply(eqdata$log_PGV, eqdata$SiteClass, length))
average_cluster_size_earthquake_id <- mean(tapply(eqdata$log_PGV, eqdata$earthquake_id, length))
average_cluster_size_year <- mean(tapply(eqdata$log_PGV, eqdata$year, length))
...
design_effect_year <- 1 + (average_cluster_size_year - 1) * ICC_year
...
print(paste("Design effect for year index:", design_effect_year))


# Random Intercept model
model_ri <- lmer(log_PGV ~ 1 + log_Pv + (1 | station_name) + (1 | earthquake_id) + (1 | year), data = train_data, REML = FALSE, control = control)
summary(model_ri)
anova(model_ri)
...
# Random slope model
model_ris_1 <- lmer(log_PGV ~ 1 + log_Pv + (1 + log_Pv | station_name) + (1 | earthquake_id) + (1 | year), data = train_data, REML = FALSE, control = control)
summary(model_ris_1)
anova(model_ris_1)
performance::r2(model_ris_1)
#dotplot(ranef(model_ris_1$station_name, condVar = TRUE))

# Source the new data testing function
source("/Users/admin/Desktop/WORK/s-wave-intensity-estimation-nz/scripts/test_new_data.R")

# Test the model_ris_1 on new data from after 2023
test_results_ris_1 <- test_model_ris_1_new_data(
  fitted_model = model_ris_1,
  new_data_path = "/Users/admin/Desktop/WORK/s-wave-intensity-estimation-nz/data/eqdata_vel_new.csv",
  training_data = train_data
)

# New function to test the model model_ris_1
# Fixed effects
...
model_ris_2 <- lmer(log_PGV ~ 1 + log_Pv + (1 + log_Pv | station_name) + (1 + log_Pv | year) + (1 | earthquake_id), data = train_data, REML = FALSE, control = control)
summary(model_ris_2)
anova(model_ris_2)
...
model_ris_3 <- lmer(log_PGV ~ 1 + log_Pv + (1 + log_Pv | station_name) + (1 + log_Pv | earthquake_id) + (1 | year), data = train_data, REML = FALSE, control = control)
summary(model_ris_3)
anova(model_ris_3)
...
library(flexplot)
simple_model <- lmer(log_PGV ~ 1 + log_Pv + (1 + log_Pv | station_name), data = train_data, REML = FALSE, control = control)
visualize(simple_model, plot = "model", sample = 288, pch = 8)
p + theme(legend.text = element_text(size = 2))
visualize(simple_model, plot = "model", formula = log_PGV ~ log_Pv | station_name, sample = 5, legend(pch = 8))
visualize(simple_model, plot = "model", formula = log_PGV ~ log_Pv + station_name, sample = 5, legend(pch = 2))

simple_model2 <- lmer(log_PGV ~ 1 + log_Pv + (1 + log_Pv | station_name) + (1 + log_Pv | earthquake_id) + (1 | year), data = train_data, REML = FALSE, control = control)
visualize(simple_model2, plot = "model", sample = 20)
visualize(simple_model2, formula = log_PGV ~ log_Pv | year, sample = 10)
visualize(simple_model2, formula = log_PGV ~ log_Pv + station_name, sample = 15)
...
# Subset schools
subsch <- sort(unique(train_data$station_name))[95:109]
# Get the data for these schools and name it as "dat"
dat <- train_data[train_data$station_name %in% subsch, c("station_name", "log_Pv", "log_PGV")]
dat$station_name <- as.factor(dat$station_name)
xyplot(log_PGV ~ log_Pv | as.factor(year),
  group = station_name, type = c("p", "r"), test_data
)

# load the lattice package
library(lattice)
# Call xyplot and add regression lines
xyplot(log_PGV ~ log_Pv | station_name, type = c("p", "r"), lwd = 3, dat)

par(mar = c(4, 4, .1, .1))
xyplot(log_PGV ~ log_Pv,
  group = station_name, type = c("p", "r"),
  auto.key = list(columns = nlevels(dat$station_name)), dat
)

par(mar = c(4, 4, .1, .1))
xyplot(log_PGV ~ log_Pv, group = station_name, type = c("p", "r"), train_data)


# Sort the data for better plotting
dSci <- eqdata[order(train_data$station_name, train_data$log_Pv), ]
# Make the plot
xyplot(log_PGV ~ log_Pv, group = station_name, type = c("p", "r"), dSci)
...
for (i in 1:num_stations) {
  ...
  current_station_id <- eqdata$station_name[i]

  # Filter data for the current station
  station_subset <- subset(eqdata, station_name == current_station_id)
  ...
  # Add legend
  legend("topright",
    legend = c("Simple Model", "Multilevel Model"),
...
subsch <- sort(unique(train_data$station_name))[95:100]
par(mfrow = c(3, 2))
for (station_name in subsch) {
  # Filter data for the current station
  station_subset <- subset(eqdata, station_name == station_name)
...
train_data <- train_data %>%
  mutate(
    pred_fixef = predict(model_ris_1, newdata = ., re.form = NA),
    pred_ranef = predict(model_ris_1, newdata = ., re.form = ~ (1 + log_Pv | station_name)),
...
# filtered_data <- train_data %>%
#   filter(station_name %in% sampled_stations)
...
station_random_effects <- as.data.frame(random_effects$station_name) %>% mutate(grp = rownames(.))
earthquake_random_effects <- as.data.frame(random_effects$earthquake_id) %>% mutate(grp = rownames(.))
year_random_effects <- as.data.frame(random_effects$year) %>% mutate(grp = rownames(.))

# Plot random effects for station_name using ggplot2
ggplot(station_random_effects, aes(x = grp, y = `(Intercept)`)) +
  ...
  labs(title = "Random Effects for station_name", x = "Station Name", y = "Random Effect") +
  theme(axis.text.x = element_text(angle = 90))

qqnorm(station_random_effects$`(Intercept)`)
qqline(station_random_effects$`(Intercept)`, col = "red")

# Plot random effects for earthquake_id using ggplot2
ggplot(earthquake_random_effects, aes(x = grp, y = `(Intercept)`)) +
  geom_point() +
  geom_errorbar(aes(ymin = `(Intercept)` - 1.96 * `(Intercept)`, ymax = `(Intercept)` + 1.96 * `(Intercept)`), width = 0.1) +
  labs(title = "Random Effects for earthquake_id", x = "Earthquake ID", y = "Random Effect") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))

# Plot random effects for year using ggplot2
ggplot(year_random_effects, aes(x = grp, y = `(Intercept)`)) +
  geom_point() +
  geom_errorbar(aes(ymin = `(Intercept)` - 1.96 * `(Intercept)`, ymax = `(Intercept)` + 1.96 * `(Intercept)`), width = 0.1) +
  labs(title = "Random Effects for year", x = "Year Index", y = "Random Effect") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
# Time-Based Plots
ggplot(eqdata, aes(x = year, y = log_PGV)) +
  geom_boxplot() +
  labs(title = "Time Series Plot of log_PGV", x = "Year Index", y = "log_PGV")

ggplot(eqdata, aes(x = year, y = log_Pv)) +
  geom_boxplot() +
...
sampled_data <- eqdata %>%
  filter(station_name %in% sampled_stations$station_name)

# Group-Based Plots
ggplot(sampled_data, aes(x = station_name, y = log_PGV)) +
  geom_boxplot() +
  labs(title = "Boxplot of log_PGV by station_name", x = "station_name", y = "log_PGV") +
...
# Anova tests
anova_model1 <- aov(log_PGV ~ station_name, data = eqdata)
summary(anova_model1)

anova_model2 <- aov(log_PGV ~ year, data = eqdata)
summary(anova_model2)

anova_model3 <- aov(log_PGV ~ earthquake_id, data = eqdata)
